...
def process_folder(folder):
    ...
    os.makedirs("lossesCOMMU", exist_ok=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--batch", type=str, default=None, help="Caminho para arquivo .txt com lista de IDs do batch")
    args = parser.parse_args()

    if args.batch:
        with open(args.batch, "r") as f:
            selected_ids = [line.strip() for line in f.readlines()]
        npz_files = [f"{track_id}.npz" for track_id in selected_ids]
    else:
        npz_files = [f for f in os.listdir(args.folder) if f.endswith(".npz")]
