...
def build_model(device: torch.device) -> DisentangleVAE:
    ...
    pnotree_decoder = PianoTreeDecoder(note_embedding=None, dec_dur_hid_size=64, z_size=512)

    # DisentangleVAE expects names (chd_encoder, rhy_encoder, decoder, chd_decoder).
    # We pass interface.py-compatible modules: txt_encoder -> rhy_encoder and
    # pnotree_decoder -> decoder.
    model = DisentangleVAE('disvae-smoke-cpu', device, chd_encoder, txt_encoder, pnotree_decoder, chd_decoder)
    return model
